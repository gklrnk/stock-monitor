...
def pobierz_wszystkie_pozycje():
    ...
    # Grupuj transakcje po tickerze i sortuj chronologicznie
    po_tickerach = {}
    for t in transakcje:
        po_tickerach.setdefault(t["ticker"], []).append(t)

    aktywne = []

    for ticker, lista in po_tickerach.items():
        lista = sorted(lista, key=lambda x: (x["data"], x["id"]))

        # Kolejka partii zakupowych FIFO: [ilosc, koszt_jednostkowy]
        lots = []
        nazwa = ticker
        waluta = "PLN"
        liczba_zakupow = 0
        ...
        for t in lista:
            nazwa = t.get("nazwa") or nazwa
            waluta = t.get("waluta") or waluta

            if t["typ"] == "KUPNO":
                ilosc = t["ilosc"]
                if ilosc <= 0:
                    continue
                # Prowizja powieksza koszt nabycia
                koszt_jedn = (t["wartosc"] + t.get("prowizja", 0)) / ilosc
                lots.append([ilosc, koszt_jedn])
                liczba_zakupow += 1

            elif t["typ"] == "SPRZEDAZ":
                do_sprzedania = t["ilosc"]
                liczba_sprzedazy += 1
                # Zdejmuj z najstarszych partii (FIFO) - usuwamy KOSZT
                # sprzedanych akcji, a nie przychod ze sprzedazy
                while do_sprzedania > 0 and lots:
                    if lots[0][0] <= do_sprzedania:
                        do_sprzedania -= lots[0][0]
                        lots.pop(0)
                    else:
                        lots[0][0] -= do_sprzedania
                        do_sprzedania = 0

        ilosc_total = sum(l[0] for l in lots)
        if ilosc_total <= 0:
            continue

        zainwestowano = sum(l[0] * l[1] for l in lots)

        aktywne.append({
            "ticker": ticker,
            "nazwa": nazwa,
            "ilosc": int(ilosc_total),
            "zainwestowano": round(zainwestowano, 2),
            "srednia_cena": round(zainwestowano / ilosc_total, 2),
            "waluta": waluta,
