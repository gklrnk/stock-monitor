...
from services.local_storage import (
    zapisz_lokalnie, wczytaj_lokalnie, usun_lokalnie, status_lokalny
)
...
def zapisz_dane(nazwa_pliku, dane):
    ...
    if not token:
        # Brak tokena - zapis lokalny (tryb offline / jedno urzadzenie)
        return zapisz_lokalnie(nazwa_pliku, dane)
...
def wczytaj_dane(nazwa_pliku, domyslne=None):
    ...
    token, username, data_repo = _get_config()
    if not token:
        # Brak tokena - odczyt lokalny
        return wczytaj_lokalnie(nazwa_pliku, domyslne)
...
def usun_plik(nazwa_pliku):
    ...
    token, username, data_repo = _get_config()
    if not token:
        return usun_lokalnie(nazwa_pliku)
...
def status_polaczenia():
    ...
    Gdy brak tokena - zwraca status lokalnego magazynu danych,
    dzieki czemu aplikacja dziala bez zadnej konfiguracji.
